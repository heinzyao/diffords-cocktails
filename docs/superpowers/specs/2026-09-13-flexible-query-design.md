...
因此：`min_count` 預設 `None`；由裸列表路徑（「社群高分精選」）
明確傳入 `min_count=5`。語意上 5 票門檻屬於「高分精選」，不屬於查詢引擎。

評分篩選路徑（`--rating` / `評分`）刻意不套這個隱藏門檻：一旦條件可疊加，
「篩選出評分 ≥ N 的酒」就該真的回傳所有評分 ≥ N 的酒，暗中再疊加票數門檻
會跟「條件可疊加」的語意互相打架，也會讓使用者以為自己選的條件與實際套用
的條件不一致。真的需要票數門檻時，請明確用 CLI 的 `--min-count` 指定。
