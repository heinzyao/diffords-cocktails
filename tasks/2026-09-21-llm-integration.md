...
### 階段 0：修 selector —— 已完成（2026-09-19）
...
三個階段全部完成並上線，另外多做了 Flavour Profile 與排程自動化。

### 最初的判斷哪些對、哪些錯

**對的：** 「機會不在語意檢索，在自然語言 → 查詢參數」。階段 1 解決的是
日常查詢，階段 2 是錦上添花。先做階段 1 的順序沒錯。

**錯的：** 把問題診斷成「review selector 壞了」。實際是網站 2026-08 全面
改版，整條 HTML 提取路徑失效。低估的原因是只抽驗了一頁就下結論。

**沒料到的：** `description` 是模板文字這件事雖然一開始就發現了，但沒想到
`review` 修好後會成為語意檢索的好素材 —— 6,744 筆真實品飲評語，跨語言檢索
效果比預期好（「苦苦的餐前酒」→ 0.877）。
...
1. 抽驗 3 頁就斷定「History / Prepare 區塊已從網站移除」 ——
   回填後發現有 5,901 / 4,992 筆，真相是新版版型不只一種。
2. 用 25 筆樣本評估語意檢索品質，分數擠在 0.72-0.81 以為區辨度不足 ——
   全量 6,744 筆後變成 0.82-0.88 且結果精準。

兩次都是「樣本不足以代表母體」。之後寫抽樣結論要標明樣本數。

### 只有實測才會發現的問題

mock 測試全綠但功能完全不可用的情況出現了三次：

| 問題 | mock 為什麼測不到 |
|---|---|
| Gemini 最小 deadline 10 秒（規劃寫 3 秒） | mock 不會驗證參數合法性 |
| tag 精確比對，LLM 猜的 "Sour" 查到 0 筆 | mock 回傳的是「正確」的 tag |
| 配額按每筆 content 計，第 30 批就撞上限 | mock 沒有配額 |

規劃階段寫下的數字（3 秒 timeout）不等於 API 接受的數字。涉及外部服務的
規格一律要實測確認。

### 測試品質

CI 抓到本機測試一直在打 GCS（`bot.py` import 時 `load_dotenv()` 讀
`GCS_BUCKET`），整套測試因此從 9.9 秒降到 0.57 秒。這個污染在加 LLM 功能
之前就存在，只是沒有測試碰到那條分支 —— 新功能意外暴露了舊問題。

另外兩個測試設計問題也一併修了：`test_query.py` 手工列舉 `argparse.Namespace`
欄位（改用真實 parser），以及 bot 層測試把 `embeddings.search` 整個 mock 掉，
導致「索引表不存在會拋例外」這個生產級 bug 只有單元測試抓得到。

### 差點造成的資料流失

`_upsert_cocktail` 原本無條件覆寫所有欄位。改版後 `prepare` 必為 None，
照原樣跑一次 full scrape 會清掉 4,645 筆。是先讀了 `save_cocktail` 才發現的 ——
如果直接照規劃改完 selector 就去回填，資料已經沒了。

### 成本

兩次 full scrape 各約 7 小時（禮貌延遲 2-4 秒，不能省）。Gemini 用量在個人
查詢量下屬於雜訊等級；建索引 6,744 筆也只是一次性的幾分鐘。
